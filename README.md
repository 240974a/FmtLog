...
**В журнале настоящее время.** Приложение сообщает библиотеке текущее время —
из NTP, из RTC или откуда угодно, — и записи получают календарную отметку
вместо счёта от запуска.
...
Текст образца библиотека не разбирает и никак не толкует — она лишь ищет в нём
`{}`. Писать в журнал можно на любом языке, в том числе по-русски:

```cpp
log::info(F("бойлер {} °C, насос {}"), 54.25, F("включён"));
```

В примерах и тестах текст английский, чтобы библиотека читалась всеми.
...
void setup() {
    ...
    log::info(F("started"));
    log::error(F("sensor is silent {} times in a row"), 3);
}
```
...
Пока время неизвестно, в первом столбце идёт счёт от запуска. Как только
приложение его сообщит, там появляется календарная дата:
...
Откуда взять время — из NTP, из микросхемы RTC или иначе — решает приложение:
...
Смещение часового пояса библиотека не знает и не прибавляет — передавайте уже
местное время, если нужно местное.

Между вызовами время досчитывается по `millis()`. Их кварц уходит на секунды в
сутки, поэтому **вызов стоит повторять** — раз в час или чаще, смотря какая
точность нужна. Повторное задание убирает накопленную погрешность, а не
складывается с прежним.

```cpp
if(millis() - lastSync > 3600000ul) {
    lastSync = millis();
    log::setTime(ntp.epoch());
}
```

Отметка времени всегда занимает 21 знак, поэтому столбцы не разъезжаются на
границе синхронизации. Приёмник может и не выводить её вовсе: в `Record`
приходят и `epochSeconds` с `epochMillis`, и `uptimeMs`.
...
Отдельный модуль `FmtTelnet.h` отдаёт журнал по telnet:
...
столько, сколько её поместилось в буфер.

**У каждого соединения своё место в истории.** Чтение ничего не стирает,
поэтому второй и третий клиент получают ту же глубину, что и первый. Если
медленный клиент отстал настолько, что кольцо его догнало, он продолжает с
самого старого и получает отметку `[N bytes lost]` — потеря видна, а не
случается молча.

Размер истории и число клиентов задаются при сборке:
...
Историю можно и не отдавать — тогда клиент увидит только то, что придёт после
подключения: `telnet::setSendHistory(false)`.
...
log::info(F("boiler {}"), Temperature{54.25f, true});
```
...
```cpp
log::info(F("command '{}'{}"), name, opt(hasValue, F(" = "), value));
```
...
| `DateTimeSortable(epoch)` | `24-02-29 12:34:56` |
| `FixedWidth(7, 3)` | `007` |
...
75 тестов: разбор образца, границы буфера, все правила вывода, уровни,
источники, приёмники, раскраска и календарное время.
