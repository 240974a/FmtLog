...
**Журнал не привязан к выводу.** Библиотека никуда не пишет сама: приёмник
задаёт приложение — порт, сеть, карта памяти или всё сразу.
...
> Сообщения с дробными на AVR обходятся дороже `snprintf` — около +700 байт:
> вывод дробных отдан `dtostrf`, и он линкуется вдобавок к уже имеющемуся
> `vfprintf`. Если дробные в журнале нужны, а флеш на исходе, это стоит
> учитывать.
...
| с дробным (`%.3f`) | в 1.25 раза медленнее | — |

Целые FmtLog печатает сам, без разбора образца во время выполнения. Дробные
отдаёт `dtostrf` — тому же коду, что зовёт `printf`, плюс расходы на обход
образца.
