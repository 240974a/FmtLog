...
## 1.2.0 — 4 сентября 2026

Предварительный выпуск.

### Календарное время в журнале

Первый столбец теперь показывает настоящее время, а не только счёт от запуска.
Откуда его взять — из NTP, из микросхемы RTC или иначе — решает приложение:
...
Между вызовами время досчитывается по `millis()`, а их кварц уходит на секунды
в сутки — поэтому вызов стоит повторять, раз в час или чаще. Повторное задание
убирает накопленную погрешность, а не складывается с прежним. Счёт не сбивается
и при переполнении `millis()` каждые ~49.7 суток.

Смещение часового пояса библиотека не прибавляет: передавайте уже местное
время, если нужно местное.

Отметка занимает 21 знак в обоих видах, поэтому столбцы не разъезжаются на
границе синхронизации. В `Record` теперь приходят `epochSeconds` и
`epochMillis` — приёмник волен выводить их по-своему или не выводить вовсе.

### Новые правила вывода

| тип | пример |
|---|---|
| `DateTimeSortable(epoch)` | `26-09-03 12:30:45` — сортируется как текст |
| `FixedWidth(7, 3)` | `007` — ведущие нули до нужной ширины |

### Текст примеров переведён на английский

Строки в примерах и тестах теперь английские, чтобы библиотека читалась всеми.
На возможности это не влияет: образец библиотека не разбирает и не толкует, а
лишь ищет в нём `{}`. Писать в журнал можно на любом языке, в том числе
по-русски.

### Что проверено

- 74 теста, из них 14 новых на работу со временем
- каждый новый тест проверен мутацией
- 9 сочетаний примеров и плат собираются
